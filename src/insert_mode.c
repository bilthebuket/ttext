...
static void handle_default(EditorState* es, int ch)
{
	...
	su_handle_insertion(&(t->su), line_index + t->x);
	if (ch == '}')
	{
		bool indent = true;
		int j;
		...
		if (indent)
		{
			...
			for (int i = 0; i < amount; i++)
			{
				...
				su_handle_deletion(&(t->su), line_index);
			}
			t->x -= amount;
		}
	}
...
static void handle_tab(EditorState* es, int ch)
{
	...
	for (int i = 0; i < TAB_SIZE; i++)
	{
		...
		su_handle_insertion(&(t->su), line_index + t->x + i);
	}
	t->x += TAB_SIZE;
	check_right_update(t);
	move_cursor_to_tab(t);
...
static void handle_backspace(EditorState* es, int ch)
{
	...
	if (t->x > 0)
	{
		...
		su_handle_deletion(&(t->su), line_index + t->x - 1);
	...
	else if (t->y > 0)
	{
		...
		if (line_above_index >= 0)
		{
			...
			su_handle_deletion(&(t->su), line_index - 1);
